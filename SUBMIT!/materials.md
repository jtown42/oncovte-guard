...
> Human-readable master reference. **Authoritative paste source for the portal is
> `submission/*.txt` (or the combined `submission/SUBMISSION-FULL.md`), which are
> verified within every limit by `npm run check:submission`.** This doc carries a
> slightly richer variant of the long-form fields for review and Q&A prep; where the
> two differ in exact wording, the `submission/*.txt` files win, but every load-bearing
> *claim* is kept in sync across both. Bracketed **[ ]** items must still be supplied by
> the student/advisor before final submission (logo, headshot, advisor attestation).
...
IMPACT. Cancer-associated VTE drives substantial morbidity, mortality, hospitalization, and cost, and it interrupts oncologic treatment; contemporary US data show a 12-month VTE incidence of ~3.7% overall and ~5.7% in patients on systemic therapy. Randomized trials (AVERT, CASSINI) and a pooled analysis of both show that targeted DOAC prophylaxis in higher-risk ambulatory patients reduces overall VTE events; conversely, anticoagulating the wrong patient - one with severe thrombocytopenia, an active bleed, or a major DOAC drug interaction - causes preventable hemorrhage. In the short term, a tool that makes the guideline-concordant choice fast and explicit can raise appropriate prophylaxis rates and prevent unsafe anticoagulation at the individual encounter. In the long term, because the logic is reusable and scales across a very large population, small per-encounter improvements aggregate into meaningful reductions in preventable VTE, VTE-related death, bleeding harm, and treatment interruption.

INNOVATION. OncoVTE Guard is distinguished by four things. First, it unifies Khorana risk scoring with a structured, source-attributed 52-agent DOAC-chemotherapy interaction knowledge base - whose recommendation-changing major interactions are each individually evidence-anchored - in a single recommendation; most tools do one or the other. Second, its contraindication logic is "appliesTo"-aware: a condition such as HIT removes only the affected agents (LMWH) while leaving DOACs preferred, instead of bluntly aborting the assessment. Third, it is dual-surface - the exact same tested reasoning engine powers both an interactive SMART on FHIR dashboard (clinician pull) and a CDS Hooks service (EHR push) - so it meets clinicians where they already work. Fourth, it treats clinical accuracy as a testable property: every rule is backed by an automated test and an auditable rule-to-source-to-code-to-test traceability matrix.
```
...
1. Cancer-site classification and exclusions. ICD-10-CM codes are matched by hierarchical prefix. Disease-specific populations (multiple myeloma, primary/metastatic brain tumor, acute leukemia, myeloproliferative neoplasms) are detected and routed to their own disease-specific pathway (e.g., IMiD-associated myeloma prophylaxis) rather than being mis-scored or silently dropped.
2. Khorana VTE risk score. Cancer site (0-2) plus four one-point criteria (platelets >=350, hemoglobin <10 or ESA use, WBC >11, BMI >=35), capped at 6. Tiers follow the original Khorana model / NCCN VTE-C (0 Low, 1-2 Intermediate, >=3 High); pharmacologic prophylaxis is indicated at >=2 - the trial-validated threshold used by ITAC and NCCN and matching the AVERT/CASSINI inclusion criteria, deliberately distinct from Khorana's original >=3 high-risk cut point (net benefit is greatest at >=3, where the number-needed-to-treat is roughly half that at a score of 2). Each criterion's boundary is explicit and tested.
3. DOAC-chemotherapy interaction screening. Every active medication is checked against a 52-agent knowledge base curated from two authoritative secondary references - the AHA 2022 Scientific Statement on cardio-oncology drug interactions and Hellfritzsch et al. 2024 - plus FDA DOAC labeling for the azole antifungals, applied consistently across all 52 agents; the 16 recommendation-changing "major" cells are individually evidence-anchored, and the knowledge base is explicit that its curation date denotes authorship, not clinician sign-off. For each agent, the engine returns the full per-DOAC profile (apixaban, rivaroxaban, dabigatran, edoxaban) with mechanism, severity, and management, and aggregates a worst-case severity (major > moderate > pharmacodynamic > minor > none > unknown).
...
QUALITATIVE. Each of the five decision states was visually verified in the running app and captured as a screenshot, checked against a stated readability rationale: severity is carried by a color-coded matrix, ranked alerts, and an explicit "avoid / not an option" list, never by color alone. Every text/background pair passes a measured WCAG 2.1 AA contrast audit (docs/ACCESSIBILITY.md). The interface has not been evaluated with clinician users; a think-aloud protocol is written and awaiting a participant.

CONCLUSIONS. The engine reproduces NCCN-concordant outputs deterministically across the tested space, and the dual-surface architecture delivers identical results through both the dashboard and the CDS Hooks service. The project met its goals for clinical accuracy and for standards-based EHR integration. It has not yet been evaluated with live EHR data or in a clinical setting; the planned prospective validation would track appropriate-prophylaxis rate, symptomatic VTE, major bleeding, and alert override/fatigue.

SUSTAINABILITY. The design is built to last and to be maintained. The DOAC interaction knowledge base is an external, versioned data file in which every agent carries source attribution, so guideline and labeling updates are data edits, not code rewrites. Clinical engines are decoupled from UI and transport, so a guideline change is localized and immediately re-verified by the test suite that encodes it. The app is built entirely on open standards - FHIR R4, SMART App Launch, and CDS Hooks - which maximizes portability across EHRs and avoids vendor lock-in. The encoded primary-prophylaxis rules are current as of NCCN v1.2026, ITAC 2022, ASCO 2023, and ACC 2026; recent developments (API-CAT reduced-dose apixaban for extended secondary prevention, biomarker-guided prophylaxis, and emerging factor XI inhibitors) sit outside this app's primary-prophylaxis scope but are tracked as future work. The roadmap is to validate against public SMART/FHIR sandboxes, broaden the knowledge base under clinical review, and, contingent on review, publish in the SMART App Gallery for community adoption and contribution.
```
...
```text
SMART on FHIR + CDS Hooks engine that scores cancer VTE risk and flags DOAC-chemo interactions, 185 tests verifying guideline fidelity.
```
...
```text
Reviewers can reproduce every claim: `npm test` runs the full 185-test suite, `npm run build` type-checks and builds, and `npm run dev` opens the standalone demo with its live "what-if" editor. Key artifacts in the repository: VERIFICATION.md (the rule-to-source-to-code-to-test traceability matrix), docs/DEMO-SCRIPT.md (an eight-minute walkthrough of all five decision states, including a live threshold-flip demonstration), and docs/screenshots/ (the five decision states). No live-demo video has been recorded yet. The app has additionally been EHR-launched against the SMART Health IT public sandbox: loaded with synthetic oncology FHIR bundles, it completed the OAuth2/PKCE handshake, read live Condition, Observation, and MedicationRequest resources off the server, and returned the expected recommend, LMWH-fallback, and contraindicated verdicts - demonstrating standards-native interoperability with the reasoning intact (a public sandbox with synthetic data; not clinician-validated).
```
...
- **Farge D, Frere C, Connors JM, et al.** 2019 International Clinical Practice Guidelines for the Treatment and Prophylaxis of Venous Thromboembolism in Patients With Cancer (ITAC). *Lancet Oncol.* 2019;20(10):e566–e581. — supports VTE as the second leading cause of death in cancer, the ~4–7-fold increased VTE risk, and cancer accounting for ~20% ("one in five") of all VTE events.
- **Farge D, Frere C, Connors JM, et al.** 2022 international clinical practice guidelines for the treatment and prophylaxis of VTE in patients with cancer (ITAC 2022). *Lancet Oncol.* 2022;23(7):e334–e347. — second-leading-cause phrasing; Khorana ≥2 prophylaxis threshold (1B); DOACs "in the absence of strong drug–drug interactions" (1A).
- **Lyon AR, López-Fernández T, Couch LS, et al.** 2022 ESC Guidelines on cardio-oncology. *Eur Heart J.* 2022;43(41):4229–4361. — second-leading-cause phrasing; ≥2 threshold.
...
- **Lam BD, Ryu J, Jafari O, et al.** Epidemiology of Cancer-Associated Venous Thromboembolism Across the United States. *Am J Hematol.* 2026;101(5):1005–1018. doi:10.1002/ajh.70271. — contemporary US CAT epidemiology (12-mo incidence ~3.7% overall, ~5.7% on systemic therapy; kidney ~7.6% vs bladder ~4.8%, testicular ~3.8%).
- **Bosch FTM, Mulder FI, Kamphuisen PW, et al.** Primary thromboprophylaxis in ambulatory cancer patients: a systematic review and meta-analysis. *Blood Adv.* 2020;4(20):5215–5225. — pooled RR ~0.51 at KS ≥2; NNT ~34 at score 2 vs ~17 at ≥3.
- **Lyman GH, Carrier M, Ay C, et al.** ASH 2021 guidelines for management of VTE: prevention and treatment in patients with cancer. *Blood Adv.* 2021;5(4):927–974. — equipoise at KS = 2.
- **Key NS, Khorana AA, Kuderer NM, et al.** VTE prophylaxis and treatment in patients with cancer: ASCO guideline update. *J Clin Oncol.* 2023;41(16):3063–3071.
- Supporting efficacy trials named in the text: **AVERT** (Carrier et al., *NEJM* 2019) and **CASSINI** (Khorana et al., *NEJM* 2019); risk model: **Khorana et al.**, *Blood* 2008; pathway: **NCCN Guidelines®, Cancer-Associated Venous Thromboembolic Disease**. DDI knowledge-base sources: **AHA 2022 Scientific Statement** (cardio-oncology drug interactions), **Hellfritzsch et al. 2024**, and FDA DOAC labeling. ACC 2026 supports switching to LMWH when strong DDIs preclude a DOAC (confirm exact locator before external use).

> Wording note: the rationale states cancer-associated VTE is the **second leading cause of death** in patients with cancer, after cancer progression itself — the guideline-standard phrasing (ITAC 2019/2022, ESC 2022). The pooled trial estimate (RR ~0.56) refers to **overall VTE** (screen-detected included); the pooled **symptomatic**-VTE reduction was not statistically significant. Describe the DDI output as "faithful to a single named source set," not "accurate" unqualified.
...
2. **Paste the References above into `AMIA-App-Competition-Submission.docx`** and clear its `(cite)` placeholders. (Figures are reconciled to the guideline-standard "second leading cause of death, after cancer progression" and "approximately 2 million"; Lam et al. 2026 is now 101(5):1005–1018, doi:10.1002/ajh.70271. Confirm the exact ACC 2026 citation locator before external use.)
